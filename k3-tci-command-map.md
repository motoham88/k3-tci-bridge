...
| `ab_copy:0` (bridge's own) | `SWT13;` | `FA;`/`FB;` | — | A>B, the radio's own switch. Measured: `FB` takes `FA` |
| `ant:0,<1\|2>` (bridge's own) | `AN1;` / `AN2;` | `AN;` | — | ~0.5 s to confirm. Refused while transmitting. Reported on band change. **An antenna switch reports `AN` more than once**: the echo guard swallows one and the next arrived as news, so `AN`/`AR` from AI2 are broadcast only when they change the state |
| `rx_ant:0,<bool>` (bridge's own) | `AR1;` / `AR0;` | `AR;` | — | Re-reads `PA` and `RA`, which are stored per RX ANT state. ~0.8 s to confirm (1.4-1.9 s when it also re-read `NB`/`NL`) |
| REV | **none usable** | — | — | `SWT12` changed nothing CAT can see — `FA`, `FB`, `IF` unmoved after a tap (measured), so its state is invisible, and the reference warns REV is a limited-access state that can answer `?;`. The web UI's REV is a press-and-hold swap of A and B through `vfo`, restored on release |
| `xfil_tap:0` (bridge's own) | `SWT29;` | `XF;`, then `BW;`/`IS;` | `XFn` = FL1-FL5 | Broadcast as `xfil:0,<n>` plus `rx_filter_band`. This station has FL3 2.7k, FL4 500, FL5 250; CAT cannot report what is fitted, and the web UI shows the slot only, since the width is already on the Filter row. **Each crystal resets the DSP width to suit itself** (measured in CW: FL4 400 Hz → FL5 250 → FL3 2700 → FL4 500, not back to 400). In CW the tap cycles FL4 → FL5 → FL3 only. AI2 reports no `XF`; the reconcile sweep reads it. **The reverse holds too: a `BW` change picks the narrowest crystal at least as wide** (measured in CW: 2700 and 1000 → FL3; 500, 400, 300 → FL4; 250, 100 → FL5), so the web UI's width slider drives the crystal and XFIL is an override |
